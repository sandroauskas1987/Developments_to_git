 ...
 * Time: 10:30
 */
// сценарий прямых продаж
class NewDirectSale extends Scenarios
{
    ...
    public function script_by_steps(){
        ...
        switch (is_array($data)){
            case !array_key_exists('location',$data):$this->location();
                break;
            case !array_key_exists('address',$data):$this->address();
                break;
            ...
            case !array_key_exists('comments',$data):$this->comments();
                break;
            //case !array_key_exists('contractor',$data):$this->contractor();
            //    break;
    ...
    public function location(){
        if(array_key_exists('message',$this->in_arr) && array_key_exists('location',$this->in_arr['message'])){
            ...
            $result=$this->backgeokode($this->in_arr['message']['location']['longitude'],$this->in_arr['message']['location']['latitude']);
            if(is_array($result)){
                foreach ($result['response']['GeoObjectCollection']['featureMember'] as $key=>$featureMember){
                    if($key==0)$_SESSION['data']['address']=$featureMember['GeoObject']['metaDataProperty']['GeocoderMetaData']['text'];
                    //Logger::getLogger('backgeokode')->log($featureMember['GeoObject']['metaDataProperty']['GeocoderMetaData']['text']);
                    }
            }
    ...
    public function address(){
        if(array_key_exists('message',$this->in_arr) && array_key_exists('text',$this->in_arr['message'])){
            $_SESSION['data']['address']=$this->in_arr['message']['text'];
            $this->script_by_steps();
        }else{
            $message=$this->textmessegeobject()."\n<i> Напишите мне Адрес </i>";
            TelegrammApi::sendMessage($this->chat_id,$message,'key_clear');
            die();
        }
    }
    public function photo(){
        if(array_key_exists('message',$this->in_arr) && array_key_exists('photo',$this->in_arr['message'])){
            ...
            if(array_key_exists('media_group_id',$this->in_arr['message'])){
                ...
                if(array_key_exists($id_mediagrop,$_SESSION) && count($_SESSION[(string)$id_mediagrop])>1){
                    TelegrammApi::editMessageText($this->chat_id,array_shift($_SESSION[(string)$id_mediagrop]),"получено..");
                }
            }
    ...
    public function stage_of_construction(){
        if(is_array($this->data) && array_key_exists('stage_of_construction',$this->data)){
            ...
            TelegrammApi::sendMessage($this->chat_id,$message,'stage_of_construction');
            die();
        }
    }
    public function activity(){
        if(is_array($this->data) && array_key_exists('activity',$this->data)){
            ...
            TelegrammApi::sendMessage($this->chat_id,$message,'activity');
            die();
        }
    }
    public function marketing(){
        if(is_array($this->data) && array_key_exists('marketing',$this->data)){
            ...
            TelegrammApi::sendMessage($this->chat_id,$message,'marketing');
            die();
        }
    }
    ...
    public function new_direct_sale_write(){
        ...
        $status=KbApi::new_direct_sale($_SESSION);
        if($status==true){
            TelegrammApi::sendMessage($this->chat_id," Новый объект успешно сохранён\n переключаюсь в меню ",'key_clear');
            Logger::getLogger('objtokb')->log($_SESSION);
    ...
    public function backgeokode($longitude,$latitude){
        $url="https://geocode-maps.yandex.ru/1.x/?apikey=f21e6450-6c06-41ef-9d8e-625f988cf8b4&geocode=$longitude,$latitude&format=json&kind=house";
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type'=>'application/json','charset'=>'utf-8','type'=>'request','Access-Control-Allow-Origin'=>' *']);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_USERAGENT , "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/530.5 (HTML, like Gecko) Chrome/2.0.172.39 Safari/530.5");
            ...
            if (strlen($result) > 1) {
                ...
                if (count($result_arr) > 0) {
                    for ($i = 0; $i < count($result_arr); $i++) {
                        $result_arr_attribut[$i] = explode(",", $result_arr[$i]);
                    }
                }
                return $result_arr_attribut;
            }
        }
        return null;
    }
    public function textmessegeobject(){
        $data=array_key_exists('data',$_SESSION)?$_SESSION['data']:array();
        if(is_array($data)) {
            $textmessege = "|<b>Новый объект (создание)</b>\n";
            $textmessege .= array_key_exists('location', $data) ? "|<b>Кординаты: </b> Получены \n" : '';
            $textmessege .= array_key_exists('address', $data) ? "|<b>Адрес: </b>" . $data['address'] . "\n" : '';
